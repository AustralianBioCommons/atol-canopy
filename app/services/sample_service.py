...
class SampleSubmissionService(BaseService[SampleSubmission, SampleCreate, SampleUpdate]):
    ...
    def get_by_biosample_accession(self, db: Session, biosample_accession: str) -> Optional[SampleSubmission]:
        """Get submission sample by biosample accession."""
        return db.query(SampleSubmission).filter(SampleSubmission.biosample_accession == biosample_accession).first()
